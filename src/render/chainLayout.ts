...
function nextStep(
  ...
  // Only turn if the tile doesn't fit in the current direction
  const halfExtent = halfExtentAlongDir(toRot, dir);
  const needTurn = !wouldFit(fwd.x, fwd.z, dir, halfExtent) || overlapsAny(fwd.x, fwd.z, toRot, existing);

  if (!needTurn) {
    return { x: fwd.x, z: fwd.z, dir, runLen: runLen + 1 };
  }

  // Try to turn to find a direction that fits
  ...
  // Fallback: just place it in the forward position even if it doesn't fit
  return { x: fwd.x, z: fwd.z, dir, runLen: runLen + 1 };
}
...
export function extensionSlot(
  ...
  // Only turn if the tile doesn't fit in the current direction
  const halfExtent = halfExtentAlongDir(newRot, dir);
  const needTurn = !wouldFit(fwd.x, fwd.z, dir, halfExtent) || overlapsAny(fwd.x, fwd.z, newRot, placements);
